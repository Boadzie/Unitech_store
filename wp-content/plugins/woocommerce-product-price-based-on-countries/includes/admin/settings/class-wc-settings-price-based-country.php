	...
	public function show_zone_messages() {
		...
		if ( $current_section == 'zones' && ! empty( $_GET['edit_region'] ) && ! empty( $_GET['updated'] ) ) {				
			...
			<div id="message" class="updated inline">
				...
				<p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=price-based-country&section=zones&add_region=1' ) ); ?>" class="button button-primary"><?php esc_html_e( 'Add a new zone', 'wc-price-based-country' ); ?></a>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=price-based-country&section=zones' ) ); ?>" class="button button-secundary"><?php esc_html_e( 'Return to zones', 'wc-price-based-country' ); ?></a>
				</p>
			</div>		
			<?php			
		}
