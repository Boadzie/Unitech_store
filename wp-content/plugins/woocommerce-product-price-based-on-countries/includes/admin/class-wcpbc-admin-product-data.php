...
class WCPBC_Admin_Product_Data {
	...
	public static function init() {
		...
		add_action( 'woocommerce_bookings_after_bookings_pricing', array( __CLASS__, 'after_bookings_pricing' ) );
	}
	...
	public static function product_options_countries_prices() {					
		...
		self::show_upgrade_pro_notice( array( 'bundle', 'subscription' ), array( 'booking' ) );		
	}
	...
	public static function product_variable_attributes_countries_prices( $loop, $variation_data, $variation ) {							
		...
		self::show_upgrade_pro_notice( array( 'variable-subscription' ) );				
	}	
	...
	/**
	 * Display the pro ad in product metabox
	 * @param array $show_if
	 * @param array $_hide_if
	 */
	private static function show_upgrade_pro_notice( $_show_if, $_hide_if = array() ) {
		if ( wcpbc_is_pro() || empty( $_show_if ) ) {
			return;
		}

		$show_if = 'show_if_' . implode( ' show_if_', $_show_if );
		$hide_if = 'hide_if_' . implode( ' hide_if_', array_merge( array( 'simple', 'variable', 'external' ), $_hide_if ) );
		...
		foreach ( $pro_support as $type => $name ) {
			$class = array();
			foreach ( $pro_support as $_type => $_name ) {
				$class[] = ( $_type == $type ? '': 'hide_if_' . $_type );
			}

			$products_name .= '<span class="' . implode( ' ', $class ) . '">' . $name . '</span>';
		}
		?>
		<div style="display:none;" class="inline notice woocommerce-message wc-price-based-country-upgrade-notice <?php echo $show_if . ' ' . $hide_if ; ?>">
			<h2><?php echo sprintf( __( 'Upgrade to %1$sPrice Based on Country Pro%2$s and get support for %1$s%3$s%2$s.', 'wc-price-based-country' ), '<strong>', '</strong>', $products_name ); ?></h2>		
			<p style="margin-top:0px;"><a target="_blank" class="button button-primary" href="https://www.pricebasedcountry.com/pricing/?utm_source=product-data&utm_medium=banner&utm_campaign=Get_Pro"><?php _e( 'Upgrade to Price Based on Country Pro now!', 'wc-price-based-country' )?></a></p>
		</div>	
		<?php 
	}
