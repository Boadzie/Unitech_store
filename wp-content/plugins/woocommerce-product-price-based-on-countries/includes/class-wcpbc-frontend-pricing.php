...
class WCPBC_Frontend_Pricing {
	...
	public static function get_price_metadata( $meta_value, $object_id, $meta_key, $single ) {
		...
		if ( $single && in_array( $meta_key, wcpbc_get_overwrite_meta_keys() ) ) {						
			...
			if ( in_array( $meta_key, wcpbc_get_date_on_sale_meta_keys() ) ) {
				// Get date on sale value
				$meta_value = self::get_sale_price_dates( $meta_value, $object_id, $meta_key );				
    ...
    */
	private static function get_sale_price_dates( $value, $object_id, $meta_key ) {		
		if ( 'manual' == get_post_meta( $object_id, self::$_meta_key_prefix . '_sale_price_dates', true ) ) {		
			$value = get_post_meta( $object_id, self::$_meta_key_prefix . $meta_key, true );
		}
		return $value;	
	}
